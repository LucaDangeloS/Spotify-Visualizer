...
export async function testToken(state: State): Promise<boolean> {
    ...
    await axios
        .get(currentlyPlaying_url, headers)
...
export async function refreshToken(state: State): Promise<boolean> {
    ...
    await axios
        .post(refresh_url, querystring.stringify(refresh_body), headers)
...
export async function fetchCurrentlyPlaying(
    ...
    await axios
        ...
        .catch((err: AxiosError) => {
            ...
            else ret = { status: ApiStatusCode.Error, data: err.response };
        });
...
async function processResponse(
    state: State,
    {
        ...
        playing,
        progress,
    }: { track: trackI; playing: boolean; progress: number }
): Promise<ApiResponse> {
    let songsInSync =
    ...
    switch (true) {
        ...
        case aux == 2 && !songsInSync:
            ret = await fetchTrackData(state, {
                track: track,
                progress: progress,
            });
            break;
        ...
        case aux == 1 && !state.trackInfo.active:
            if (songsInSync) ret = { status: ApiStatusCode.VizOff, data: null };
            else
                ret = await fetchTrackData(state, {
                    track: track,
                    progress: progress,
...
async function fetchTrackData(
    state: State,
    { track, progress }: { track: trackI; progress: number }
): Promise<ApiResponse> {
    ...
    await axios
        .get(trackAnalysis_url + track.id, headers)
        .then((response: AxiosResponse) => {
