...
export function analogous(
    color: string | chroma.Color | number,
    a: number
): { left: string; right: string } {
    if (color === undefined || color === null) return null;
    let c = chroma(color);
    return {
        left: c.set("hsl.h", `-${a}`).hex(),
        right: c.set("hsl.h", `+${a}`).hex(),
    };
}
 ...
 */
export function makeTimeTransitionOffset(
    palette: string[],
    color: string,
    index: number,
    time: number,
    tickrate: number,
    timeFactor: number = 0.6
): string[] {
    if (tickrate <= 0) tickrate = 5;
    ...
    let transition: string[] = generateColorPalette(
        [color, palette[new_index]],
        false
    ).colors(steps);
 ...
 * @returns {string[]} Color transition array in hex format
 */
export function makeDistanceTransitionOffset(
    palette: string[],
    color: string,
    index: number,
    colorSteps: number = null
): string[] {
    if (colorSteps === null || colorSteps <= 0) {
        colorSteps = Math.round(
            chroma.distance(
                palette[index],
    ...
    let steps: number = Math.floor(
        chroma.distance(palette[index], color) / colorSteps
    );
...
export function pc(colors: string[], python_mode: boolean = false): string {
    ...
    for (let i = 0; i < colors.length; i++) {
        if (python_mode) console.log(`"${colors[i]}",`);
        else console.log(colors[i]);
    }
}
...
export function splitChroma(
    colors: string[],
    threshold: number = 23
): { underThreshold: string[]; overThreshold: string[] } {
    let ret_1: string[] = [];
    let ret_2: string[] = [];
    for (let i = 0; i < colors.length - 1; i++) {
